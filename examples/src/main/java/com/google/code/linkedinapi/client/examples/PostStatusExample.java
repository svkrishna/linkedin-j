...
public class PostStatusExample {
    ...
    private static void processCommandLine(CommandLine line, Options options) {
        if(line.hasOption(HELP_OPTION)) {
    		...
    		if (line.hasOption(STATUS_TEXT_OPTION)) {
        		...
        		System.out.println("Your status has been deleted. Check the LinkedIn site for confirmation.");
    		}
        } else {
            printHelp(options);
        }
    ...
    private static void printHelp(Options options) {
        ...
        String footer = MessageFormat.format("\nYou can specify only one of -{0} or -{1} options.", STATUS_TEXT_OPTION, DELETE_OPTION);
